...
How to verify: the first real `wrangler deploy`.
...
Performed by the owner, at deployment, **before enforcement is trusted**. No step
prints a credential.

1. **Confirm the endpoint (A1).** Trigger one scheduled run and read the webhook
   payload. A `stage: "cdt-query"` error means the endpoint or the signature
   method is wrong (A2), not that traffic is zero.
2. **Confirm the unit (A3).** Compare the `trafficGB` in the payload against the
   ...
   unit is not bytes.
3. **Confirm the summation scope (A5).** Compare `trafficGB` against the console
   total for the intended regions, and inspect the per-region breakdown in the
   payload.
4. **Confirm the observed state.** Verify `ecsStatusBefore` matches the console for
   the managed instance.
5. **Confirm no unintended mutation.** With traffic deliberately below the
   threshold, confirm the instance is left running and the action is a `none-*`
   no-op.
6. **Measure CPU (A8).** Read the Cron invocation's `cpuTime` and record the
   required plan in the deployment documentation.
7. **Only then** lower the threshold or treat enforcement as authoritative.

Until steps 1–3 pass, treat the traffic figure as **unverified**, not as a
measurement.
