...
| **R4** — `Traffic` unit is an assumption (bytes) | **DOCUMENTED; BLOCKED_BY_LIVE_DEPLOYMENT** | Conversion isolated in one named function, `trafficBytesToDecimalGb`, with `BYTES_PER_DECIMAL_GB`. Assumption A3; verification is comparison against the console. | **The one risk whose failure could cause an incorrect action rather than an abort** — a wrong unit stops early or enforces late. Stated as such in the register. | — |
